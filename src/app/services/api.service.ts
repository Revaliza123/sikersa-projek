...
const requestApi = (responseFields: any = null, baseUrl = undefined) => {
  ...
  axiosInstance.interceptors.request.use(async (config: any) => {
    const { credentials, loggedInUser } = store.getState().auth

    if (credentials?.access) {
      config.headers.Authorization = `Bearer ${credentials.access}`
    }

    if (loggedInUser?._id) config.headers.Uid = `${loggedInUser?._id}`
...
const getCancelToken = () => axios.CancelToken.source().token
