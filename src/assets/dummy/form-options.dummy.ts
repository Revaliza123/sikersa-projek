export const JENIS_KELAMIN = [
  { value: 'laki-laki', label: 'Laki-Laki' },
  { value: 'perempuan', label: 'Perempuan' }
]

export const AGAMA = [
  { value: 'islam', label: 'Islam' },
  { value: 'katolik', label: 'Katolik' },
  { value: 'kristen', label: 'Kristen' },
  { value: 'budha', label: 'Budha' },
  { value: 'hindu', label: 'Hindu' },
  { value: 'konghuchu', label: 'Konghuchu' }
]

export const PENDIDIKAN_TERAKHIR = [
  { value: 'sd', label: 'SD' },
  { value: 'smp/mts', label: 'SMP / MTs' },
  { value: 'sma/smk/ma', label: 'SMA / SMK / MA' },
  { value: 'd3', label: 'Diploma III' },
  { value: 'd4', label: 'Diploma IV' },
  { value: 's1', label: 'Strata I' },
  { value: 's2', label: 'Strata II' },
  { value: 's3', label: 'Strata III' }
]

export const KEWARGANEGARAAN = [
  { value: 'wni', label: 'WNI' },
  { value: 'wna', label: 'WNA' }
]

export const STATUS_PERKAWINAN = [
  { value: 'belum kawin', label: 'Belum Kawin' },
  { value: 'kawin', label: 'Kawin' },
  { value: 'cerai hidup', label: 'Cerai Hidup' },
  { value: 'cerai mati', label: 'Cerai Mati' }
]

export const PEKERJAAN = [
  { value: 'swasta', label: 'Swasta' },
]

export const DUSUN = [
  { value: 'dusun', label: 'Dusun' },
  { value: 'KOPO', label: 'Kopo' },
]

export const HASIL_PENCOCOKAN = [
  { value: 'cocok', label: 'Cocok' },
  { value: 'tidak_cocok', label: 'Tidak Cocok' },
]

export const STATUS_HAK_TANAH = [
  { value: 'sendiri', label: 'Sendiri' },
]
export const STATUS = [
  { value: 'PMA', label: 'PMA' },
  { value: 'PMDN', label: 'PMDN' },
  { value: 'Lain-Lain', label: 'Lain-Lain' },
]

export const BULAN = [
  { value: 'januari', label: 'Januari' },
  { value: 'februari', label: 'Februari' },
  { value: 'maret', label: 'Maret' },
  { value: 'april', label: 'April' },
  { value: 'mei', label: 'Mei' },
  { value: 'juni', label: 'Juni' },
  { value: 'juli', label: 'Juli' },
  { value: 'agustus', label: 'Agustus' },
  { value: 'september', label: 'September' },
  { value: 'oktober', label: 'Oktober' },
  { value: 'november', label: 'November' },
  { value: 'desember', label: 'Desember' },
]

export const STATUS_HAK_TANAH_2 = [
  { value: '-', label: '-' },
  { value: 'Hak Milik', label: 'Hak Milik' },
  { value: 'Hak Guna Bangunan', label: 'Hak Guna Bangunan' }, 
  { value: 'Hak Pakai', label: 'Hak Pakai' },
  { value: 'Hak Guna Usaha', label: 'Hak Guna Usaha' },
  { value: 'Hak Pengelolaan', label: 'Hak Pengelolaan' },
  { value: 'Hak Milik Adat', label: 'Hak Milik Adat' },
  { value: 'Hak Verponding Indonesia', label: 'Hak Verponding Indonesia' },
  { value: 'Tanah Negara', label: 'Tanah Negara' },
]